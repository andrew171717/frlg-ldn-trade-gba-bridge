...
# --------------------------------------------------------------------------------------------------
def base85_encode(data: bytes) -> bytes:
    """Encode bytes using the same custom base85 as base85_decode (inverse).

    Alphabet = ASCII 0x23..0x78 with 0x5C ('\\') skipped.
    Each 4-byte little-endian group -> 5 chars, low digit first.
    Input must be a multiple of 4 bytes.
    """
    if len(data) % 4 != 0:
        raise ValueError(f"base85_encode: length {len(data)} is not a multiple of 4")
    out = bytearray()
    for i in range(0, len(data), 4):
        ...
        for _ in range(5):
            digit = value % 85
            value //= 85
            # Map digit -> alphabet byte (skip 0x5C)
            c = digit + 0x23
            if c >= 0x5C:
                c += 1          # shift everything at or above the gap up by 1
            chars.append(c)
        out += bytes(chars)     # low digit first (same order as decode reads)
    return bytes(out)
